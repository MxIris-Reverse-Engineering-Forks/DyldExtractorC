...
struct BindRecord {
    ...
    int64_t addend = 0;
    BindRecord(uint64_t addr = 0, uint8_t tp = 0, uint8_t flgs = 0,
               int libOrd = 0, char *symName = nullptr, int64_t add = 0)
    : address(addr), type(tp), flags(flgs),
    libOrdinal(libOrd), symbolName(symName), addend(add) {}
};
